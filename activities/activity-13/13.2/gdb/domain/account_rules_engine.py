...
class AccountRulesEngine:
    ...
    _RULES: dict = {
        "SAVINGS": {"min_balance": 1000.0, "interest_rate": 4.0, "overdraft_limit": 0.0},
    ...
    def get_minimum_balance(cls, account_type: str) -> float:
        ...
        return cls._RULES.get(normalized_type, {}).get("min_balance", 0.0)
    ...
    def get_interest_rate(cls, account_type: str) -> float:
        ...
        normalized_type = account_type.strip().upper() if account_type else ""
        return cls._RULES.get(normalized_type, {}).get("interest_rate", 0.0)
    ...
    def get_overdraft_limit(cls, account_type: str) -> float:
        ...
        normalized_type = account_type.strip().upper() if account_type else ""
        return cls._RULES.get(normalized_type, {}).get("overdraft_limit", 0.0)
