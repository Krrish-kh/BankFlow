...
class IAccount(ABC):
    ...
    def get_daily_transfer_limit(self) -> float:
        """Get the maximum transfer limit for this account type and tenure tier today."""
        pass

    @abstractmethod
    def get_remaining_daily_transfer_limit(self) -> float:
        """Return the remaining amount for today's transfer limit."""
        pass

    @abstractmethod
    def can_transfer(self, amount: float) -> bool:
        """Check whether the specified amount fits within the daily transfer limit."""
        pass

    @abstractmethod
    def update_daily_transfer_total(self, amount: float) -> None:
        """Record a completed transfer against today's running total."""
        pass

    @abstractmethod
    def reset_daily_transfer_if_needed(self) -> None:
        """Reset the daily transfer counter if a new day has started."""
        pass

    @abstractmethod
    def get_daily_transfer_total(self) -> float:
        """Return the total already transferred today."""
        pass

    @abstractmethod
    def get_last_transfer_date(self) -> Any:
        """Return the date/time of the most recent transfer update."""
        pass
    ...
    def getLastTransferDate(self) -> Any: return self.get_last_transfer_date()
