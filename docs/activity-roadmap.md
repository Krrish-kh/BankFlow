...
| 03 | `activity-03-account-operations` | Lifecycle, PIN, and balance safeguards |
...
| 15 | `activity-15-funds-transfer` | Secure transfers and tenure-based daily limits |
...
1. I make one bounded capability change on its activity branch.
2. I run that activity's tests and the relevant regression checks.
3. I merge the verified branch into the next cumulative stage.
4. I keep `main` aligned with the latest integrated activity.

This keeps the history readable while allowing any feature stage to be inspected on its own.
